// tests/konformitaet/vectors/wycheproof_hmac_sha256_vectors.mjs
//
// Test vectors from C2SP/wycheproof project.
// Apache License 2.0. https://github.com/C2SP/wycheproof
// Vectors extracted from testvectors_v1/hmac_sha256_test.json
// source.version laut Datei: "google-wycheproof" 0.9
// Beschafft am 19.08.2026 (Archiv ohne beiliegenden Commit-Hash — anders als bei
// den 2026-04 uebernommenen Dateien ist hier darum KEIN Commit angegeben; die
// Herkunftsangabe ist die der Datei selbst, nicht eine erfundene Genauigkeit).
//
// Bezug: Zerfall in Feld-Einheiten (A339, 19.08.2026). Die pseudonymen Adressen der
// Feld-Einheiten werden per HMAC-SHA256 ueber den Feldnamen abgeleitet; bis zum
// 19.08.2026 pruefte das Repo HMAC gegen KEINE externen Vektoren.
// Verwendung: Vivodepots HMAC-Nutzung (Web Crypto sign/verify) gegen externe
// Edge-Case-Vektoren validieren.
// Result-Codes: "valid" (must succeed), "invalid" (must fail),
// "acceptable" (either is OK — keine Pass-Fail-Bewertung, ADR-055 D3).
//
// Algorithmus: HMAC mit SHA-256. Schema: mac_test_schema_v1.json.
// Inputs: key, msg (Hex). Output: tag (Hex, tagSize/8 Byte).
// Bei tagSize 128 ist der erwartete Tag die ERSTEN 16 Byte des vollen HMAC.
//
// UEBERNAHME: byte-unveraendert je Testvektor. Die GRUPPEN-Felder der Quelldatei
// (keySize, tagSize) sind in jeden Vektor hineingezogen, weil diese Datei eine
// FLACHE Liste exportiert — dieselbe Form wie die vorhandenen Wycheproof-Module.
// Ohne sie waere ein Vektor nicht auswertbar: die Tag-Laenge steht nur dort.

export const wycheproofHmacSha256Vectors = [
  {
    "keySize": 256,
    "tagSize": 256,
    "tcId": 1,
