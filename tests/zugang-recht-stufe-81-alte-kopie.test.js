'use strict';
/* Gerüst-Schnitt S5 (20.09.2026): der Zugang-Auszug ist kein Kern-Auszug mehr, das Produkt trägt ihn als Template. Ein Depot
   auf Schema 80 trägt eine Kopie von damals (alte Kennungen). Eigene Module gewinnen bei gleicher Kennung immer
   (`_logikModuleAlle`), die alte Kopie würde also das Modul des Produkts verdecken — dauerhaft und lautlos in den Daten der
   Bürgerin. Stufe 81 räumt sie ab, wenn das öffnende Produkt das Modul trägt; sonst bleibt sie (der Bürgerin bliebe keines).
...
const TEMPLATE = path.join(__dirname, 'fixtures', 'zugang-zum-recht-beratungshilfe-logikmodul.json');

function kernMit(zusatz) {
  const p = PRODUKTE.find((x) => x.slug === 'privat-de');
  const ziel = fs.mkdtempSync(path.join(os.tmpdir(), 'zugang-81-'));
  try {
    const dateien = modulDateienFuer(p).filter((f) => !/zugang-zum-recht/.test(f)).concat(zusatz);
...
const depotAufSchema80 = () => ({ schemaVersion: 80, logikModule: [alteKopie()], sektoren: {} });

test('[Stufe 81] trägt das Produkt das Template, wird die alte Kopie entfernt und das Modul des Produkts greift', () => {
...
test('[Stufe 81·Rot-Beweis] trägt das Produkt das Template nicht, bleibt die alte Kopie stehen — der Bürgerin bliebe sonst keines', () => {
  const V = kernMit([]);
  const d = depotAufSchema80();
  V.depotNormalisieren(d);
...
test('[Stufe 81·Rot-Beweis] eine Kopie, die nicht von Vivodepot stammt, bleibt auch bei tragendem Produkt unangetastet', () => {
  const V = kernMit([TEMPLATE]);
  const d = depotAufSchema80();
